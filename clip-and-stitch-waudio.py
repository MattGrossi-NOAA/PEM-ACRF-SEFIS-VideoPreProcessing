...
def process_deployments(config_path='configurations.yml'):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Total Progress"):
        ...
        for i, f_info in enumerate(needed_files):
            ...
            # Trim BOTH video and audio segments and reset their clocks 
            # (setpts/asetpts) to 0 so the stitcher sees a clean sequence 
            # starting from 0.0s
            v_label = f"[v{i}]"
            a_label = f"[a{i}]"
            filter_complex_parts.append(
                f"[{i}:v]trim=start={f_info['ss']}:duration={trim_duration},setpts=PTS-STARTPTS{v_label};"
                f"[{i}:a]atrim=start={f_info['ss']}:duration={trim_duration},asetpts=PTS-STARTPTS{a_label}"
            )
            # Concat requires inputs in [v0][a0][v1][a1]... order
            filter_inputs += f"{v_label}{a_label}"

        # Concatenate trimmed segments. n=number of files; v=1 video; a=1 audio out.
        ...
        #   -map: select the video output and audio output from the filter
        #   -c:a: audio codec to use for the output
        cmd = [
            ffmpeg_exe, "-y"
        ] + input_args + [
