...
A `configurations.yml` file is an easy way to control tool behavior as well as keep a record of the settings used each time. The most important settings in this file to pay attention to are:
...
The configuration file is a YAML file structured as `key: value` pairs. The order in which these entries are presented in the file does not matter, but [all keys need to be included and match exactly](index.qmd#options) as expected. For example:
...
The disadvantage to this option is that it will require the full directory path to be included when [running the model](usage.qmd#running-the-model) if executing in command line and specifying the configuration file to use.
